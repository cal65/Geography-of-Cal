...
plot(G2$Date.Begin, rep(-10, length(G2[,1])), ylim=c(10,60), ylab='Cities', xlab='Date', main='Travels of Cal')
...
map<- ggplot() + mapWorld
mapPoints <- map + geom_point(aes(x = long, y = lat, size =(TN^0.5)/12), colour=strptime(G3$Date.Begin, format='%Y-%m-%d')$year+2000, data = G3, alpha = .5)+ theme(legend.position='right') + scale_color_discrete(name="Year")
mapPoints + scale_size(range=c(0,4), guide=FALSE) + ggtitle('Cal Travel Map') + labs(x='', y='') +theme(axis.ticks=element_blank())
